 ...
 */
// might have closed too many file descriptors should try and do them for the different cases
void start_program(Program *programs, int num_programs, int cur) {
    // TODO

    //if(pipe(p)== -1){
        //die("pipe() failed."); // check for pipe failure


    pid_t cpid = fork(); // create the child
    //programs[cur].pid = cpid; // save the cpid to the program structure

    if(cpid < 0){ // check if fork failed
        die("fork() failed.");
    }
    else if(cpid == 0){ // in the child
        if (cur != num_programs - 1) { //if you're not at the last thing because the last thing should be stdout so you don't want to close it
            dup2(programs[cur].fd_out, 1); // swap stdout with fd_out
            //close(programs[cur].fd_out); // close stdout
        }


        if(cur > 0){
            dup2(programs[cur].fd_in, 0); // swap fd_in with stdnin
            //close(programs[cur].fd_in); // close stdin
        }
        for (int j = cur; j < num_programs; j++) { // for loop wil close all of the programs immediately after dup2 and before execvp statement
            ...
            //dup2(programs[cur].fd_in, 0); // swap fd_in with stdin
            //close(programs[cur].fd_in); // close stdin
            //dup2(programs[cur].fd_out, 1); // swap fd_out with stdout
            //close(programs[cur].fd_out); // close stdout
        }
        //close(p[0]); // close the read end of the pipe
        //close(p[1]); // close the write end of the pipe
        execvp(programs[cur].argv[0], programs[cur].argv); // upgrade the child
        die("execvp() failed"); // execvp failed
    }
    else{ // in the parent
        programs[cur].pid = cpid;
        if (cur > 0 && programs[cur].fd_in > 0) {// not first program
            close(programs[cur].fd_in); // the first program only dup2 once so we close that
        }
        if (cur != num_programs - 1 && programs[cur].fd_out > 0 ){//not last program, not first either
            close(programs[cur].fd_out); // last one dup2 on stdin and fd_in
        }
       /* else{ // inbetween
            close(p[0]);
            close(p[1]);
...
int wait_on_program(Program *prog){
    ...
    // TODO
    int cpid = prog->pid; // get the pid of the child of this Program
    waitpid(cpid, &exitStatus, 0); // wait for the child to end
    // no need for a die statment because exitStatus will handle this
...
void prepare_pipes(Program *programs, int num_programs){
    int p[2]; // make an array of length 2
    for (int j = 1; j < num_programs; j++) {
        if (pipe(p) == -1) {
            die("pipe() failed");
        }
        programs[j - 1].fd_out = p[1];
        programs[j].fd_in = p[0];
    }
