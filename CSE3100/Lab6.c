...
{
    srand(seed); // set the number to start creating random numbers at, the min random number
...
char * gmn_get_message(gmn_t *pg) // return whether we guessed right or not
{
    return pg->message;
}

int    gmn_get_max() // return the maxvalue that the guess can be
{
    return MAX_VALUE;
}
